...
import { FiActivity, FiPlus, FiExternalLink, FiCheck } from "react-icons/fi";
...
export default function ChangeFeedPage() {
  ...
  const fetchAces = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/aces?limit=50");
      ...
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted">
            View all AI change events (ACE) captured from your systems
          </p>
        </div>
        <button
          type="button"
      ...
      {justCreatedId && (
        <div className="mb-6 p-4 rounded-xl border border-gold-500/30 bg-gold-500/10 flex flex-wrap items-center gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Link
              href={`/app/change-feed/${justCreatedId}`}
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {items.map((ace) => (
            <Link
              key={ace.ace_id}
              href={`/app/change-feed/${ace.ace_id}`}
        ...
        <div className="p-8 rounded-xl border border-white/14 bg-gradient-to-b from-white/8 to-white/4 text-center">
          ...
          <p className="text-muted mb-6">
            Log a change manually below—no integrations needed—or set up an
            integration to capture ACEs automatically.
          </p>
